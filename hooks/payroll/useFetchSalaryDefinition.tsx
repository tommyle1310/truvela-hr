...
import { Props_SalaryDefinition } from "@/types/screens/payroll/salary-definition"; // Adjust this import as per your type structure
import axiosInstance from "@/utils/axiosConfig";
import { useCallback, useEffect, useState } from "react";

const useFetchSalaryDefinition = () => {
    const [listSalaryDefinitions, setListSalaryDefinitions] = useState<Props_SalaryDefinition[]>([]);
    ...
    const fetchSalaryDefinitions = useCallback(() => {
        ...
        axiosInstance
            .get<ApiResponse>('/salary-definitions') // Fetch salary definitions data from the API
            .then((response) => {
                if (response.data.EC === 0) {
                    setListSalaryDefinitions(response.data.data); // Store the fetched salary definitions in the state
                } else {
    ...
    return { listSalaryDefinitions, isLoading, refetch: fetchSalaryDefinitions, error };
};

export default useFetchSalaryDefinition;
